...
public class Graph {
	...
	public static void main(final String[] args) throws IOException {
		 ...
		 * File[] fileList = xmlPath.listFiles(); int i=0; for(File file :
		 * fileList) { i++; if(i>=0) { String fname = file.getAbsolutePath();
		 * System.out.println("Processing "+fname + " : " + i); Transaction tx1
		 * = graphDb.beginTx(); try { //if(fname.contains("general_")==true)
		 * populate(fname); tx1.success(); } finally { tx1.finish(); } } }
		 */
		shutdown();
	}
	...
	private static void insertParameter(RelationshipType outgoing, RelationshipType incoming, ClassElement type,
			Node node, int paramIndex) throws IOException {
		if (type == null)
			return;
		// System.out.println(type.getId());
